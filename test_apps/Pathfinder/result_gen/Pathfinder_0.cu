...
void run(int argc, char **argv) {
    ...
    fprintf(stderr, "GPUFI_OUTPUT base=0x%llx bytes=%llu name=Pathfinder_result\n",
            (unsigned long long)(uintptr_t)gpuResult[final_ret],
            (unsigned long long)(sizeof(int) * cols));
    fflush(stderr);
